...
public class save_newuserrole: IHttpHandler,System.Web.SessionState.IRequiresSessionState
{
	...
	public void ProcessRequest(HttpContext context)
	{
		...
		try
		{
			...
			DataTable ppdet_listTable = new DataTable("ppdet_list");

            ppdet_listTable.Columns.Add("ctrl_id", typeof(String));
            ppdet_listTable.Columns.Add("ctrl_value", typeof(String));
            ppdet_listTable.Columns["ctrl_value"].AllowDBNull = true;

            TabDetail tab_detail;
            foreach (var tdetail in objDataRequest.context.TabDetail)
            {
                tab_detail = JsonConvert.DeserializeObject<TabDetail>(JsonConvert.SerializeObject(tdetail, Formatting.None, settings));
                //logResult(context, JsonConvert.SerializeObject(tdetail, Formatting.None, settings));

                DataRow ppdet_listdr = ppdet_listTable.NewRow();
                ppdet_listTable.Rows.Add(ppdet_listdr);

                ppdet_listdr["ctrl_id"] = tab_detail.ctrl_id;
                ppdet_listdr["ctrl_value"] = tab_detail.ctrl_value;
            }
				...
				{	
					...
					using (SqlCommand cmd = new SqlCommand(sCommand , conUsrRole))
					{
						try
						{
							...
							cmd.Parameters.Add("@ppdet_list", SqlDbType.Structured).Value = ppdet_listTable;
